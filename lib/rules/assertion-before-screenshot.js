...
module.exports = {
  ...
  create(context) {
    return {
      CallExpression(node) {
        if (isCallingCyScreenshot(node, context) && !isPreviousAnAssertion(node, context)) {
...
function getCallExpressionCypressCommand(node, context) {
  return isRootCypress(node, context)
    && node.callee.property
    && node.callee.property.type === 'Identifier'
    && node.callee.property.name
}

function isCallingCyScreenshot(node, context) {
  return getCallExpressionCypressCommand(node, context) === 'screenshot'
}

function getPreviousCypressCommand(node, context) {
  ...
  if (index === 0) return getPreviousCypressCommand(node.parent, context)
  ...
  return getCallExpressionCypressCommand(previousStatement.expression, context)
}

function isPreviousAnAssertion(node, context) {
  const previousCypressCommand = getPreviousCypressCommand(node, context)
